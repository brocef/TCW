...
Run the **Doctor** procedure in this plugin's `skills/tcw-plugin/SKILL.md`: locate
`tcw` and its package source, detect an editable (`pip install -e`) dev install and
leave it alone, find the active cache version (sibling-dir scan with `sort -V`), and
