...
| 5 | `tcw work stage prompt plan $SLUG` on a configured node contains every entry's path, trigger and description, and does **not** mention the agent guide. |
| 6 | `tcw work stage prompt implement $SLUG` likewise. |
| 7 | `tcw work stage prompt spec $SLUG` — a stage carrying no span — contains **none** of the entry paths. |
...
`tcw work stage prompt plan` output on an unconfigured node against that **same
