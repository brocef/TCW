...
- `tcw work stage gate <stage> <slug>` and `tcw work stage prompt <stage> [<slug>]`, and `--no-exec` on each
...
| # | Assertion |
| - | --------- |
| 1 | On an **unconfigured** node, `tcw work stage prompt spec $SLUG` exits 0 and prints TCW's built-in instructions. |
...
| 14 | `--no-exec` on each verb reports only that verb's own half and executes none of it — `gate` its `pre` checks, `prompt` its bindings — proven by having each create a sentinel file and asserting neither exists. Both print nothing on stdout. |
