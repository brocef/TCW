...
export function buildPathTree<T>(items: T[], keyOf: (item: T) => string): Array<TreeNode<T>> {
  const map = new Map<string, TreeNode<T>>();
  const itemByKey = new Map<string, T>();
  for (const item of items) if (!itemByKey.has(keyOf(item))) itemByKey.set(keyOf(item), item);
  for (const key of itemByKey.keys()) {
    let cursor = "";
    for (const segment of key.split("/")) {
      cursor = cursor ? `${cursor}/${segment}` : segment;
      if (!map.has(cursor)) map.set(cursor, { name: segment, path: cursor, item: null, children: [] });
    }
...
function resolveParentKey(childKey: string, parentRef: string, keys: Set<string>): string {
...
export function buildWorkTree(items: WorkItem[]): Array<TreeNode<WorkItem>> {
  ...
  for (const item of items) {
    if (!index.has(item.slug)) {
      index.set(item.slug, { name: item.title ?? item.slug, path: item.slug, item, children: [] });
    }
  }
  ...
  for (const [key, node] of index) {
    ...
    const parentKey = parentRef ? resolveParentKey(key, parentRef, keys) : "";
...
export function pruneTree<T>(
  ...
  const visit = (node: TreeNode<T>): TreeNode<T> | null => {
    const children = node.children.map(visit).filter((child): child is TreeNode<T> => child !== null);
    if (children.length) {
  ...
  return { nodes: nodes.map(visit).filter((node): node is TreeNode<T> => node !== null), forceExpand };
}
