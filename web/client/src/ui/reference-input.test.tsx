...
test("exposes combobox state and selects canonical identifiers by keyboard", () => {
  ...
  const { rerender } = render(<ReferenceInput label="Feature" options={options} value={value} onChange={(next) => { value = next; }} />);
  const input = screen.getByRole("combobox", { name: "Feature" });
  fireEvent.change(input, { target: { value: "use" } });
  expect(input).toHaveAttribute("aria-expanded", "true");
  expect(screen.getAllByText("use", { selector: "strong" }).length).toBeGreaterThan(0);
  fireEvent.keyDown(input, { key: "Enter" });
  expect(value).toBe("web/use");
  rerender(<ReferenceInput label="Feature" options={options} value={value} onChange={(next) => { value = next; }} />);
});

test("supports pointer selection, duplicate-free raw text, and negated conditions", () => {
  let value: string | string[] = [];
  const { rerender } = render(<ReferenceInput multiple negated label="When" options={options} value={value} onChange={(next) => { value = next; }} />);
  ...
  rerender(<ReferenceInput multiple negated label="When" options={options} value={value} onChange={(next) => { value = next; }} />);
