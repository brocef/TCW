...
`tcw work list --status active` → `tcw work show <slug>` → read the item's `content.md` plus whatever lifecycle artifacts exist (`initial-request.md`, `spec.md`, `plan.md`, `outcome.md`, `refined-outcome.md`). For an epic, `tcw work reconcile <slug>` to refresh the rollup before choosing the next action.
