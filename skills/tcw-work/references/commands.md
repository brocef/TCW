...
| read a procedure         | `tcw work procedure prompt <id> [<slug>] [--no-exec]` — one of TCW's procedures (`unattended-work`, `triage-issues`, `documentation-sync`, `post-mortem`, `create-work`, `audit-backlog`, `consolidate-plans`, `decompose`, `delegation`, `search`) composed with the project's `work.procedures` text; plain text, no header, no gate. The slug is optional and lets `when:` and `generate:` see that item |
