...
get their procedure from `tcw work procedure prompt triage-issues` and
`post-mortem`, today's text as the default. The forge question (whether `gh`
becomes something a project can replace or stays declared and required) is
answered in `spec` with its reason. The post-mortem's stage ladder stays fixed;
`agents/tcw-post-mortem.md` stops carrying its own copy of the skill's
procedure.
