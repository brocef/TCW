...
1. **`tcw-setup` never says how to run the install under Codex.**
   `skills/tcw-setup/references/install.md` says "Under Codex there is no hook, and you
...
2. **`tcw-work-stage`'s manual fallback writes `<plugin>` for the plugin folder** with no
...
3. **One skill, two capabilities.** `work/run-a-lifecycle-stage` still has two paragraphs
   about the `tcw-work-stage` skill, which now has its own capability,
   `skills/tcw-work-stage`. Wanted: what those paragraphs say that the skill's capability
...
4. **The issue-closing rules are written twice**, in `work/complete-a-work-item` and
   `skills/tcw-extras-triage-issues`. Wanted: one home, linked from the other.
