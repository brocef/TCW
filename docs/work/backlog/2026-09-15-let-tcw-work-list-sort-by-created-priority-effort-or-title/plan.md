...
5. `_render_descendant_boards` (`:571`): take and pass through
   `sort`/`order`. When `sort` is set, after `entries` is built (`:588-592`)
...
- **AC16** `--help` output (via `SystemExit` from `main(["work", "list",
  "--help"])`) contains `--sort`, `{created,priority,effort,title}`,
  `--order`, `{asc,desc}` and the per-key defaults.
...
- `docs/release-notes/upcoming.md` **[Public-API] — fires.** A short
  plain-language entry: `tcw work list --sort` with the four keys,
...
- `docs/changelogs/upcoming.md` **[Any-Code-Change] — fires.** Under **Added**:
  `tcw work list --sort {created,priority,effort,title}` and
  `--order {asc,desc}`; `WORK_SORT_KEYS`, `WORK_SORT_ORDERS`,
  `WORK_SORT_DEFAULT_ORDER`, `order_by`, and `WorkStore.board(sort=, order=)` in
...
- `skills/<component>/SKILL.md` **[Skill-Driven-Component] — fires** for
  ...
  created|priority|effort|title [--order asc|desc]]` — hides resolved; `-i` adds
...
- The requester settled the direction questions by asking for explicit
  ...
  Z in either direction). There are no open questions.
