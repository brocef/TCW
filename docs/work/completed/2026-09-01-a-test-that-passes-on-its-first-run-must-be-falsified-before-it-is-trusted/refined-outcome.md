...
- **Version.** Folded into `v1.2.0` rather than given one of its own. That tag
  was cut at the epic's closeout and never pushed, so `scripts/unpushed-version.sh`
  reported `FOLDABLE` and the release could still grow to cover this. The
  procedure was followed as written: tag deleted, the `upcoming.md` content merged
  **into v1.2.0's existing sections** rather than appended as rival ones, the
  upcoming files reset, and the tag re-created at `HEAD`. The version-bearing
  files were not touched, because a fold does not bump. Re-running the gate now
  reports nothing left to fold, which is the state that says it worked.
