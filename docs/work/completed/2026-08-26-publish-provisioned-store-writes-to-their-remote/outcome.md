...
**What the review actually covered, and what it did not.** The first invocation
was an unbounded "review this diff" brief. It exited 0 after 6523 lines of file
dumps with no findings section at all — a *non-answer that reads as success*, and
it was re-run rather than trusted. The second was bounded to five specific
questions and answered all five with a verdict. So **child C's diff never received
a whole-diff independent review**; it received a targeted five-question one, plus
the systematic attack run by hand in this session.

That limit does not weaken the findings, because none of them rest on the tool:
each claim was checked against the code before being acted on, one was proved
wrong (`UnicodeDecodeError` is a `ValueError` subclass and was already caught) and
one was judged benign. It does mean the areas nobody asked about are unreviewed,
and a reader should not take "independently reviewed" here to mean more than the
...
## Addendum — the whole-diff review, done later with `bllm`

The gap recorded above was closed after the fact. Codex could not review the
whole diff; `bllm review diff` could, in **slices** — the local models are slow
and have a much smaller context window, so one 879-line brief is not an option.
Three slices, by concern rather than by hunk, each with its own guidance file
naming the contract and the attacks to try:

| Slice | Raised | Survived verification |
| ----- | ------ | --------------------- |
| the publication gate (`resolve_store`, `__init__`, `_open_at`, `publishes`) | 3 | **1** |
| the publish path (`publish`, `_publish_branch`, `_publish_after_transition`) | 1 | 0 |
| the refresh path (`ensure_available`, `_refresh`, `refresh`, `_diverged`) | 4 | 0 |

Seven of eight rejected, each with evidence rather than by dismissal: `stdout`
cannot be `None` under `capture_output=True`; `refresh` already guards
`counts.stdout.split()` before indexing it; `_run` raises `ValueError` so the
handler does catch it; a deleted remote branch is simply recreated by the push,
tested; and `git fetch` updating remote-tracking refs is not a change to the
*item*, which is what the criterion says.

**The one that survived is not in this item's code.** With both a `work.path`
naming an unusable directory and an unprovisioned declaration, `tcw validate`
reports only the declaration — two configuration problems, one reported, by a
command whose whole job is enumerating them. That is child A's resolution ladder,
untouched here, and four `codex exec` rounds over child A never found it. Filed as
[its own item](tcw://W/2026-09-01-a-broken-work-path-is-hidden-when-a-repository-is-also-declared)
rather than reworked into a completed and released item.

**A slice can be wrong, not merely incomplete.** The gate slice's first run
reported a `TypeError`/`AttributeError` that would have broken every publishing
store — a serious finding, and false. The extraction had grabbed
`FsTreeStore.__init__` instead of `FsWorkStore.__init__`, so the reviewer reasoned
correctly about code that was not the code under review. Re-run on a corrected
slice it found **no defects** and walked all three ladder rules confirming the
safety property. The lesson is not "distrust the reviewer" — it is that with
slicing, *verifying the slice* comes before verifying the findings.
