...
- **The reviewing tool is unreliable in a way that reads as success.** The
  whole-diff brief exited 0 with no findings; only the bounded five-question brief
  answered. An empty or truncated result from it means *no answer*, not *no
  findings*, and treating the two as the same would have accepted this item on a
  review that never happened. See `outcome.md` for what was and was not covered.
