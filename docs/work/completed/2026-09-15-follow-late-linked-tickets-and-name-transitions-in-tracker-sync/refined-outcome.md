...
Recorded by hand. The acceptance above was of the design before review. After
review, and by the user's decision, catching a late-linked ticket up is opt-in through
`tcw work tracker link --sync-status`, and a plain link warns and leaves the ticket
alone. GitHub #42 is resolved in that shape: no false conflict, and an explicit way to
bring the ticket along. The deferrals above — issues stay open until the fix is
published, no version cut — still stand.
