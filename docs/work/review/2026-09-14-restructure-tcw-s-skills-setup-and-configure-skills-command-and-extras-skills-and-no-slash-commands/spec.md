# Spec — Restructure TCW's skills: setup and configure skills, command and extras skills, and no slash commands

> **About this spec**
>
> - **Where it came from.** It is carved from the spec of
>   `2026-09-14-consolidate-the-setup-skills-into-a-single-tcw-setup-skill` as it
>   stood at commit `9fbaa25a`, after that item's three review rounds. The
>   requester split the work in three. This item is the skill restructure.
>   - The eval harness changes are in
>     `2026-09-14-make-eval-checks-measure-what-the-agent-did-…`, which blocks this
>     item, because this item's eval cases use its predicates and fixture.
>   - The taxonomy Features and one capability per skill stay in the original
>     item, which this item blocks.
> - **The requester's decisions** are revision notes 1–15 in the original item's
>   `initial-request.md`, plus the round-three answers recorded in this item's
>   `initial-request.md`.
> - **A large change is acceptable**, because the goal is cleanup.

## Capability changes

The ledger is restructured in the taxonomy-and-capabilities item. This item
touches only what would otherwise be false once it ships:
...
- **`plugin/bootstrap-the-cli`.** Its two mentions of `tcw-plugin` become
  `tcw-setup`. The behavior it describes is unchanged.
- **`work/run-a-lifecycle-stage`.** The paragraph at `description.md:104-116`
  introduces the five per-stage skills and ends "`tcw-work-stage` still reaches
  both, which is why it stays". It is reworded to say that `tcw-work-stage` reaches
  every stage, under Claude and Codex alike, with the stage named in the request
  (D8).
- **Left for the taxonomy-and-capabilities item.** Nine capabilities that item
  deletes still name removed commands or skills. Between this item and that one,
  their bodies are stale on purpose, and AC 4 excludes `docs/capabilities/`. They
  are:
...
- **Taxonomy:** no change in this item.
...
- Any change to the `tcw` CLI.
- Taxonomy Features and per-skill capabilities: the original item.
- Eval harness predicates, the fixed `files_changed_exactly`, and the `bare`
  fixture: the eval-checks item.
...
| | `tcw-work-stage` | now the only stage skill, and works fully under Codex (D8) |
| | `tcw-post-mortem` | its command deleted (D9); the skill is unchanged |
...
| **Extras** | `tcw-extras-autonomous-work` | renamed from `autonomous-work` (D8) |
...
- **`tcw-work`, more** (round three)
  - `references/commands.md:31-32`: delete the `tcw-work-stage-<id>` row, and drop
    "**Claude only.**" from the `tcw-work-stage` row (D8).
  - `references/commands.md:42-43`: keep both rows as pointers to the procedures,
    and replace "`/tcw-audit-work-backlog` in Claude" and
    "`/tcw-consolidate-plans` in Claude" with "ask the `tcw-work` skill"
    (the requester's round-three answer).
- **`documentation-sync`**
  ...
  - `references/release-notes-and-changelogs.md:5` ("read `setup.md`") gets the
    same change.
  - **`/tcw-cut-version`'s two unique points move in** (round three, D9):
    - `references/cut-version.md` gains, before its step that rotates
      `upcoming.md`: "Write the release-note and changelog entries into the
      `upcoming.md` files first. The cut rotates them."
    - `cut-version.md:3` and the `SKILL.md` table row at `:99` also open the
      document when the user asks directly to cut a version, not only after
      picking a bump from the completion options.
- **`tcw-config.yaml`**
...
**1–2. Eval infrastructure comes from the eval-checks item.** That item adds the
`tool_input_contains` and `tool_input_absent` predicates (checks that look only at
what the agent actually ran or opened), fixes `files_changed_exactly` so it
compares against the seeded commit, and adds named fixture variants with a `bare`
repository. The D3 rule, pointers in words and never paths, keeps skill text from
satisfying `tool_input_*` checks by accident.
...
3. **New `tcw-setup` case: initialize a fresh repository**, on a new `bare`
   ...
   - **Assertions:**
     ...
     - `validate_exit_zero`.
   - **Fixture:** `"fixture": "bare"`, from the eval-checks item.
...
5. **A1–A4 and A8 invoke `tcw-work-stage`** instead of the deleted per-stage skills
   ...
   exists. Its reason still holds for the `request` route, so it moves into a new
   `PARTIAL["tcw-work-stage"]` entry rather than being lost.
...
- A deleted-names test, following `DELETED` at
  ...
  exist.
- Mutation checks: each new or changed assertion in B4, B5, B8, B11 and B12 gets a
  grading test with a hand-built transcript that must fail it, alongside the
  eval-checks item's predicate tests.
...
| `README.md:440` | The `tcw-plugin` row → rows for `tcw-setup` and `tcw-configure`, in the same commit that deletes `tcw-plugin`, so the deleted-names test passes at that commit. |
| `README.md:34`, `:230`, `:463` | Table-of-contents entry, install sentence and heading that say "slash commands" → "command skills". |
| `.claude-plugin/marketplace.json:12` | "skills + slash commands" → "skills". |
| `evals/assets/gen_requirement.py:35`, `tests/test_eval_fixture.py:149` | Comments saying "which every per-stage skill documents" → "which `tcw-work-stage` documents". |
...
| `docs/capabilities/plugin/bootstrap-the-cli/description.md`, `docs/capabilities/work/run-a-lifecycle-stage/description.md` | Per **Capability changes**, edited as body text. |
| `docs/changelogs/upcoming.md`, `docs/release-notes/upcoming.md` | Every removed command and skill, the renames, the two new skills and four command skills, and the new eval cases. |
...
- **Make `tcw-work-stage` work fully under Codex** (the requester's round-three
  answer). Its fallback block (`skills/tcw-work-stage/SKILL.md:41-47`) gains one
  sentence before the commands: "Use the stage and work item named in the request
  in place of `$stage` and `$item`." Each per-stage skill used to carry this with
  the stage written in.
...
| `tcw-cut-version` | Deleted, after moving two points into `cut-version.md` (D4) | "Use the project's own version-cut process first" → `skills/documentation-sync/references/cut-version.md:14`; "ask before pushing" → `:86`. Round three found two points not covered: "write the `upcoming.md` entries before cutting", and reaching the document from a direct "cut a version" request. D4 adds both. |
...
- **`skills/tcw-work/references/commands.md:135-140`** ("Slash commands (Claude
  ...
  keep pointing at the procedures, with the slash-command mentions replaced (D4).
...
- **`skills/tcw-extras-autonomous-work/SKILL.md:3`** (its `description`, "via
  ...
  both become the `tcw-commands-drive-work-to-completion` skill.
...
- **Abstraction litmus test.** No store operation is added or changed. This item
  moves and renames agent-facing documents only.
...
15. **Capabilities.**
    ...
    - `tcw capabilities show work/run-a-lifecycle-stage` prints none of the five
      `tcw-work-stage-<stage>` names.
    - `tcw capabilities check` prints `capabilities OK`.
16. **Round-three fixes.**
    - `skills/tcw-work/references/commands.md` has no `tcw-work-stage-<id>` row,
      and its `tcw-work-stage` row does not contain `Claude only`.
    - `skills/tcw-work-stage/SKILL.md` contains `in place of`, `$stage` and
      `$item` in one sentence.
    - `skills/documentation-sync/references/cut-version.md` contains `entries into
      the`, `upcoming.md` and `first` in one sentence.
    - `.claude-plugin/marketplace.json` and `README.md` do not contain
      `slash command`.
17. **Evals.**
    ...
    - `PARTIAL` has key `tcw-work-stage`, carrying the reason for the `request`
      route.
    - `tests/test_eval_grading.py` has, for each assertion B11 and B12 add, a
      grading test on a hand-built transcript that must fail it.
18. `tcw work docs` lists an entry whose path starts
    `skills/tcw-configure/references/`.
19. Bare `pytest` passes (how CI runs it), and `tcw validate` exits 0.
...
- **Text is split across skills.** An agent in `tcw-work` reading
  ...
  "both" and "below".
- **Stale capability bodies between items.** Until the taxonomy-and-capabilities
  item lands, nine capabilities describe removed commands and skills (Capability
  changes). That is accepted; AC 4 excludes them explicitly, so the gap is
  visible.
- **Dependency.** The eval-checks item must complete first.
...
- **Open items whose referenced files or capabilities change.**
  - `2026-09-11-run-the-eval-harness-and-act-on-what-it-finds`: this item lands
    first. The note lists the new cases B11 and B12, the retargeted B5, A1–A4 and
    A8 (with the axis A baseline risk above), the four command-skill exclusions,
    and the new `PARTIAL` entries.
  ...
  - `2026-08-12-separate-the-agent-plugin-from-the-python-cli-source`: its
    `plan.md` names `tcw-plugin` paths, and moves `commands/`, which this item
    deletes.
- **Axis A's baseline moves.** Grading never reads `invokes`, so retargeting A1–A4
  and A8 is bookkeeping. But the agent now has to pass two arguments to
  `tcw-work-stage` instead of one. If it passes only the item, `$stage` takes the
  item's value, the injected blocks fail quietly, and nonce results shift from
  `injected` to `fallback`. The run-the-evals item's note records this (round
  three).
...
- **Size.** This is the largest of the three items: effort very high, complexity
  high.

## Notes

- **Reviews.** The content here went through three multi-review rounds as part of
  the original item. Round three's findings are applied: D4, D7, D8, D9, ACs 4 and
  16, and Risks. The requester chose no fourth round.
