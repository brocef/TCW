...
2. Repair stale Playwright expected artifacts exposed only after the serial
   suite passes the original failure, verifying each against current DOM and
   model evidence.
3. Run `python -m pytest -q`, `pnpm test`, `pnpm test:e2e`, and `pnpm lint`.
   Confirm every Playwright scenario executes rather than being skipped after a
   serial-suite failure.
4. Re-evaluate Documentation Sync against the finished diff. This test-fixture
