...
Starting work is an atomic claim, and a claim TCW cannot record is refused before the item moves — outside a Git repository the item stays in the backlog and the command says why. `--worktree` needs the *code* project in a repository as well as the work store, since that is where the worktree and the `.gitignore` rule for it land; a plain start needs only the store. I identify the claimant with `--owner`,
