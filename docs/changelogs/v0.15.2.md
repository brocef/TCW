...
- `skills/documentation-sync/scripts/unpushed-version.sh` — the gate for the new
  ...
  finds it on the upstream's remote **or** `git branch -r --contains` shows its
  commit already rode out on a remote branch; unpublished if there is no remote
  at all. Exit `0` foldable / `1` not / `2` remote unreachable — the agent reads
  the code, never the prose, and `2` is deliberately not collapsed into `1` so an
  offline check can't be misread as "safe to rewrite the tag".
...
- `tests/test_unpushed_version_script.py` — seven cases over tmp_path repos with
  a local bare remote (no network): no tag, tag at HEAD, unpushed with later
  commits, pushed tag, unpushed tag whose commit is on the remote branch,
  unreachable remote, and the executable bit.
