...
- **`docs/work/dod.yaml`** — this node's own Definition of Done: the five
  ...
  because the file **replaces** the defaults rather than extending them — a
  dropped entry deletes a check from every completion with no error and no
  failing test. Harness parity comes free: `dod_checklist()` is declared on the
  abstract store (`tcw/store/base.py:983`), so any adapter serves it.
- **`skills/tcw-triage-issues/SKILL.md` §8** — the closeout half of the loop.
  ...
  rule rather than cross-referencing §6, since a reader arriving at closeout has
  not necessarily read it.
- **Capability `work/customize-the-definition-of-done`** (`cap-73460f`) — the
  DoD override, which worked but appeared in no README, skill, doc, or ledger
  entry, and which `work/complete-a-work-item` positively contradicted.
...
- `docs/capabilities/work/complete-a-work-item` — "the same fixed checklist on
  every item" was wrong; the checklist is node-configurable. Corrected, and
...
- The issue-closeout change carries **no `tcw/` diff**. The request asked whether
  it finally justified a `source`/`external-ref` field on the work model; it does
  not. Provenance is body content, and body is already one of the four things the
  abstract model says an item has — a field would buy machine-readable lookup for
  a single grep at a single moment, performed by an agent already reading the
  item. The DoD also stays `[prompted]`, never `[gated]`: enforcement would
  require `tcw` to make a network call at completion, and `tcw work complete`
...
- Verified by running rather than reading: this repo prints six checklist items
  ...
  discard prints none, only the permanence warning; and `self.root / "dod.yaml"`
  does resolve to `docs/work/dod.yaml`, which was that spec's one open
  assumption.
