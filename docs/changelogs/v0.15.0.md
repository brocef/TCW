...
ranges so entries trace back to source. Blocks are chronological.
...
<changes starting-hash="f8f95d5" ending-hash="4de80a2">
...
- **`WorkItem.pr`** — pull-request URL, persisted in `state.yaml`, set with
  ...
  by `show` when non-empty. **Removed again later in this same release** — see the
  final block; it never reached a published version, and the release notes do not
  mention it.
...
<changes starting-hash="e34f082" ending-hash="282435e">
...
<changes starting-hash="36193dd" ending-hash="2e3d918">
...
- **`LIFECYCLE_STEPS`** — the machine-readable contract for every stage and
  transition (objective, inputs, produced artifact, status move, gates). The
  stage documents must agree with it; one source makes that checkable.
...
- The web complete modal states that configured hooks do not run there, and that
  ...
  terminal. Carried over from the auto-commit child's deferred item — same surface.
- `work/cli.py`'s `SUBCOMMANDS` gains `submit`, `rework`, and `lifecycle`. The
  first two were an omission from the review-status child; latent, since work's
  `DEFAULT_SUBCOMMAND` is `None`, but wrong data that would misdispatch if that
  changed.

</changes>

<changes starting-hash="494eec9" ending-hash="494eec9">
...
<changes starting-hash="d77145d" ending-hash="d77145d">
...
- **`WorkItem.pr`**, added earlier in this same release. It was introduced on the
  ...
  Because v0.16.0 was folded into this release before publication, the field was
  never public: `README.md`'s `--pr` example is removed with it.

</changes>

## Release engineering

Not hash-wrapped: these are the fold itself, made in the release commit.

- **v0.16.0 was folded into v0.15.0.** Neither tag was ever pushed — the remote's
  newest was v0.14.1 — so the two cut versions were an artifact of local release
  bookkeeping, not two releases anyone could install. `docs/{release-notes,changelogs}/v0.16.0.md`
  are merged into the v0.15.0 files and deleted, and all five version-bearing
  files return to `0.15.0`.
- The changelog blocks above were normalized to the canonical
  `<changes starting-hash= ending-hash=>` form. The v0.16.0 file had drifted to
  `## Added (\`a..b\`)` headings and **hard-coded `HEAD` as the end of five ranges**,
  which stopped meaning anything the moment the next commit landed. Each is now
  pinned to the commit that actually ended that block's work.
- **`scripts/cut_version.py` retitles the rotated file** to `# v{version}`.
  Rotation was a bare `git mv`, so every released document kept the `# Upcoming`
  placeholder it was drafted under — 84 of the 92 files shipped before this
  release are titled "Upcoming". Those are left alone; the fix stops the bleed.
