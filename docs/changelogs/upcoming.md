...
- `2085163..HEAD` — Added the storage-neutral `ProjectRegistry` model and
...
- `2085163..HEAD` — Added the `connected-project-registry` taxonomy Feature and
  associated it with the eight affected capabilities.

## Changed

- `2085163..HEAD` — `tcw init` and component init mirrors accept `--id`; new and
  legacy nodes require it, configured nodes reuse their existing ID, and
  backfill preserves other config.
- `2085163..HEAD` — Work topology, qualified refs, boards, coordination, web
  routes, rollups, inbox origin metadata, and `tcw://` work namespaces use
  registered project IDs.
- `2085163..HEAD` — Taxonomy and capability `extends` are explicit project-ID
  lists resolved through the graph; the source ID is the inherited namespace.
- `2085163..HEAD` — `tcw validate` always validates the complete current project
  graph even when component scanning is narrowed.

## Removed

- `2085163..HEAD` — Removed topology discovery by directory walk, git ignore
...
- `2085163..HEAD` — Migrated topology, federation, reference, CLI, web, and
