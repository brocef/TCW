...
- **Skill authoring (progressive disclosure):** a `skills/<name>/SKILL.md` is a **thin router** — keep always-relevant judgment inline (the core lifecycle, the gates) and push genuinely rare sub-procedures into `skills/<name>/references/*.md` read on demand, each reached by a clear gate condition in the router (the `tcw-work` skill is the pattern). Only split once a skill's conditional detail is large enough to earn the indirection — for ~50-line, mostly-always-relevant skills it's a no-op; leave them inline.
...
- **When asserting a user-facing message, assert that the message it replaces
  ...
  of this one. A divergence test asserted `"diverged" in err`, matched _git's_
...
- **Watching a test go red is not enough — read _why_ it is red.** Three tests
