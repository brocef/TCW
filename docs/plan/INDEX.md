...
| 4 | [phase-4-shared-core](phase-4-shared-core.md) | extract the common bounded-tree store primitive | ✓ built |
| 5 | [phase-5-work](phase-5-work.md) | `tcw work` + `FsWorkStore`, single-node state machine + DoD gate | spec ✓ · build ✓ |
| 6 | [phase-6-beyond](phase-6-beyond.md) | cross-node recursion, skill layer, remote adapters, tracker sync | ☐ deferred (by design) |

**Where we are now:** **the single-node core is built** — Phases 1–5 are all green. `tcw` installs and exposes `init | taxonomy | capabilities | work`; the three components (`FsTaxonomyStore`, `FsCapabilitiesStore`, `FsWorkStore`) sit on the shared `FsTreeStore` core; `pytest` runs 47 tests green. **Phase 6 (beyond) is deferred by design** — it is the "nothing built until the single-node core is real and in use" bucket (cross-node recursion, the skill layer, remote adapters, tracker sync). The natural next step is to **dogfood** `tcw work` on TCW's own `docs/work/` before scheduling any Phase-6 item.
