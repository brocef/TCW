...
Release notes for v0.1.0. Plain language — no jargon or internal module names.

## New Features

Work items can now span repositories. Any folder that is a git repo with a
`docs/work/` is a **node**, and nodes nest: a repo inside another is a "child,"
the outer one its "parent."
