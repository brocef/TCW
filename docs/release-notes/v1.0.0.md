...
**TCW is 1.0.** The lifecycle model this release settles — the stages, the
documents each one produces, and the way you bind your own instructions to them
— is now something you can build on rather than something still moving under
you.

The headline: **TCW now tells you what to do at each stage itself.** Before this
release that knowledge lived only in an agent plugin, so it reached Claude users
and nobody else. Now `tcw work stage <stage> <item>` prints it, on a project that
has configured nothing, whichever agent you drive it with. `tcw work scaffold`
gives you a starting draft for any lifecycle document — one that the board
deliberately does not mistake for the document itself.

## Breaking change — one, and only if you configured it

An **empty** prompt list is now refused by `tcw validate`. If you have
`prompt: []`, or a stage id with nothing but `[]` under it, that is the one thing
to change: write `prompt: [{blob: ""}]` if you meant "this stage should say
nothing", or delete the line if you meant nothing in particular. **Everything
still runs either way while you fix it** — only `tcw validate` complains. Full
explanation under "Everything you have configured already keeps working" below.

Upgrading from 0.21.x? [**docs/migration-guide-0.21.X-to-1.0.0.md**](../migration-guide-0.21.X-to-1.0.0.md)
is the short version: the one break, then the handful of behaviours that changed
underneath you, then everything optional.

Nothing else you have configured changes, and that is checked against recordings
of the old behaviour rather than asserted.
...
## Declaring which documents must move with your code
...
guide. Each entry names a file, when it needs updating, and what to write there:
...
work:
    documentation:
        - path: README.md
          trigger: Public-API
          description: Public-facing overview and CLI usage.
        - path: docs/changelogs/upcoming.md
          trigger: Any-Code-Change
          description: Developer changelog; technical, grouped by category.
```

TCW checks the list for mistakes, and the planning and implementation steps show
it to your agent directly instead of relying on it to go and find one.

**`tcw work docs`** prints the list. It changes nothing and is safe to run at any
time; `--json` gives the machine-readable form.

**If you declare nothing, nothing changes.** The planning and implementation
instructions read exactly as before, word for word — pinned against a copy of the
old output captured before any of this was written.

## Commands no longer hang when a script runs them

If the program that started `tcw` left its own input open — a shell wrapper, a CI
job, a git hook — `tcw work new` used to wait forever for input that was never
coming, and a batch of commands that each take a fraction of a second would
simply never finish. It now creates the work item without intake and says so.
The same fix covers `tcw work delegate`, `tcw work escalate`, `tcw taxonomy add`
...
arriving and then stops partway, the command stops with an error instead of
quietly keeping the half it received — a partial document saved as an item's raw
intake looks identical to one you wrote on purpose. Set `TCW_STDIN_TIMEOUT` to a
number of seconds to wait longer, or `0` to never wait at all.

**A lifecycle hook can no longer stall a transition by reading input it was never
given.** Hooks run with their input closed, so a hook that happens to read from
stdin finishes immediately instead of consuming the text you piped in, or timing
out and aborting the transition.

## The web view no longer offers a tab it cannot open

If a document existed but was empty, `tcw serve` listed it as written and then
failed to open it when you clicked. The list and the tab now agree: an empty
document reads as not yet written, in both places.
